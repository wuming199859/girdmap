...
{                     
  ...
  //obstacle_radius = 0.1;
  private_nh.param<bool>("debug",debug,true);
...
void AugmentedGridMap::addPointCallback(const geometry_msgs::PointStamped &added_point)
{
  ...
  //添加
 // std::cout<<"map.size():[" << original_map.data.size() << "]\n";
  //std::cout<<"map[100].data:[" << original_map.data[100] << "]\n";
...
//
  ...
  ROS_INFO("Adding point on: [%f,%f]",added_point.point.x,
    ...
    //可注释
  // int n=0;
  // while(n<original_map.data.size()){
  //   if(original_map.data[n] == 100){
...
void AugmentedGridMap::addObstacleToMap(geometry_msgs::PointStamped added_point)
{
  ...
  obstacles.push_back(added_point.point);
        ...
        //添加输出
  // std::cout<<"x_orig:[" << x_orig<<"]\n";
  // std::cout<<"y_orig:[" << y_orig<<"]\n";
  std::cout<<"obstacle_radius:[" << obstacle_radius<<"]\n";
  ...
  int target = 2; //区域内enhanced_map.data[ ]==100点的阈值 大于则认为没有漏检
  int count =0;
  for (int i = min_x; i < max_x ; i++)
  {
    for (int j = min_y; j < max_y ; j++)
    {
      ...
      if(enhanced_map.data[i+j*map_metadata.width]==100){
        count++;
      }
    }
  }
if(count<target){
    std::cout<<"当前区域未内检出阈值以上的点是占据的！fail! \n"; 
}else{
    std::cout<<"当前区域内检出阈值以上的点是占据的！ success!\n"; 
}
point++;
  ...
  // int ans_0=0;
  // while(n<original_map.data.size()){ 
  //   if(enhanced_map.data[n]==0){
  //     ans_0++;  
  //   }  
  //   n++;
  // }
  // enhanced_map.data[300] = 100;
  // std::cout<<"ans_100:[" << ans_100 << "]\n"; 
  // std::cout<<"ans_0:[" << ans_0 << "]\n";
  // std::cout<<"enhanced_map.data:[" <<enhanced_map.data[10] << "]\n";
...
void AugmentedGridMap::makeObstaclesMarkers()
{
  ...
  //enhanced_map
  obstacles_markers.header.frame_id = original_map.header.frame_id;
